{
  ...
  outputs =
    ...
    let
      ...
      # the libghostty repo + rev come straight from Cargo.toml; the ghostty
      # commit is the GHOSTTY_COMMIT constant in that crate's build.rs (the only
      # place it is recorded). Fetched by rev so there is no hash to keep in sync.
      ...
      ghosttyCommit =
        let
          ...
          afterDecl = builtins.elemAt (pkgs.lib.splitString ''GHOSTTY_COMMIT: &str = "'' buildRs) 1;
        in
        builtins.head (pkgs.lib.splitString "\"" afterDecl);
